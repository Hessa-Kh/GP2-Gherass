...
class ProductionDateSelector extends StatefulWidget {
  final ValueChanged<String> onDateSelected; // Correct function type

  const ProductionDateSelector({super.key, required this.onDateSelected});
...
class _ProductionDateSelectorState extends State<ProductionDateSelector> {
  ...
  List<int> getYears() => List.generate(50, (index) => DateTime.now().year - index);
  ...
  void _selectDate(int day, int month, int year) {
    ...
    // Format the selected date as a string and pass it to the parent widget
  ...
  Widget _buildDropdown(String label, List<int> items, int selectedItem, ValueChanged<int> onChanged) {
    return Expanded(
      child: Column(
